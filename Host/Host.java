...
public class Host implements Runnable
{
	...
	{
		byte Y[] = new byte[128/8];
		new SecureRandom().nextBytes(Y);
		BigInteger y = new BigInteger(Y).abs();
		cipher.init(Cipher.ENCRYPT_MODE, PW);
		byte E[] = cipher.doFinal(g.modPow(y, p).toString().getBytes());
		
		System.out.println("waiting for connection...");
		
		socket.receive(packetr);
		
		addr = packetr.getAddress();
		...
		BigInteger ex = new BigInteger(new String(cipherr.doFinal(packetr.getData()),0,packetr.getLength()));
		
		K = new SecretKeySpec(hash.digest(ex.modPow(y,p).toString().getBytes()), "RC4");
